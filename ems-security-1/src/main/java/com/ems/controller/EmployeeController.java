...
/*@PreAuthorize(value="hasRole('ROLE_ACCOUNTANT')&& authentication.name='ravi' ")*/
public String addEmployeePage(Model model)
{
	EmployeeModel empmodel=new EmployeeModel();
	model.addAttribute("emodel",empmodel);
	return "addEmployee";
}
...
public String saveEmployee(@ModelAttribute("emodel")EmployeeModel employeeModel,Model model)
{
	...
	if(flag==true)
	{
		model.addAttribute("message","Sorry employee already exist on this given id");
		return "index";
	}
	...
	{
		repo.save(employee);
		model.addAttribute("message","Employee data added in database");
		return "index";
	}
}
...
public String deleteEmployee(@RequestParam("id")Integer empno, Model model)
{
